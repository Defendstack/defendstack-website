...
const CardSection = ({ title, items }: { title: string; items: SectionItem[] }) => (
  <section className="my-10">
    <h2 className="text-2xl font-bold mb-4 text-primary">{title}</h2>
    <div className="grid md:grid-cols-2 gap-5">
      {items.map((item, i) => (
        <div key={i} className="bg-gray-50 dark:bg-gray-800 rounded-xl p-6 shadow transition hover:shadow-md">
...
const SuccessStoryDetail: React.FC<SuccessStoryDetailProps> = ({
  ...
  <div className="bg-gradient-to-b from-white to-blue-50 dark:from-darkmode dark:to-gray-900 min-h-screen pb-16">
    <div className="max-w-3xl mx-auto pt-32 px-4 text-center">
      ...
      {subtitle && <div className="text-xl text-gray-600 dark:text-gray-400 mb-4">{subtitle}</div>}
      <div className="flex flex-col items-center gap-2 mb-10">
        <span className="text-base text-gray-500 dark:text-gray-400">{author}</span>
        {company.website && (
    ...
    {quote && (
      <div className="max-w-2xl mx-auto mb-8 px-4">
        <blockquote className="border-l-4 border-primary pl-4 italic text-lg text-gray-800 dark:text-gray-200 mb-2">
          “{quote}”
        </blockquote>
        {quoteAuthor && (
          <div className="text-primary text-right font-medium">{quoteAuthor}</div>
        )}
      </div>
    )}

    <div className="max-w-3xl mx-auto bg-white dark:bg-gray-900 rounded-2xl shadow-lg p-10">
      ...
      <section className="my-10">
        ...
        <p className="text-gray-700 dark:text-gray-300">{finalThoughts}</p>
      </section>
    </div>
